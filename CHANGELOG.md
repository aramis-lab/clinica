...
- [`oasis3-2-bids/nifd-2-bids`] Change code for backward compatibility with pandas 1.1.x
...
- [`oasis3-2-bids`] Add converter
- [GitHub] Add citation file

### Changed

- [`adni-2-bids`] Improve fetching of participants
- [`adni-2-bids`] Image path finder more robust
...
- [`adni-2-bids`] Prevent crash when files exists
- [`adni-2-bids`] Revert behavior to encode Dementia as AD
- [`adni-2-bids`] Remove entries with incoherent session names
- [`nifd-2-bids`] Several bugfixes and enhancements
...
- [`adni-2-bids`] Optimization of `adni-2-bids` clincal data extraction
- [`adni-2-bids`] Replace xlsx by tsv files forclinical data specification
...
- [`oasis-2-bids`]: Remove FSL library depency for OASIS-to-bids conversion.
- [Clinica]: Replace exception by warning when CAPs folder not recognized.
- [`aibl-2-bids`]: Center output nifti files of AIBL.
- [`aibl-2-bids`]: Extracts DICOM metadata in JSON files.
