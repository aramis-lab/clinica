...
## Outputs
Results for the template creation are stored in the following folder of the [CAPS hierarchy](../../CAPS/Specifications): `subjects/sub-<participant_label>/long-<long_label>/freesurfer_unbiased_template/sub-<participant_label>`, where <long_label> is an identifier defined by concatenating all the sessions associated with the current <participant_label> (e.g. if the template for participant `sub-01` is built from sessions `M00`, `M01`, `M05`, then <long_label> will be `M00M01M05`).
This folder contains the standard output structure of the `recon-all` command, i.e. folders such as `label/`, `mri/`, `surf/`, etc.

Results for the longitudinal processing are stored in the following folder of [CAPS hierarchy](../../CAPS/Specifications):
