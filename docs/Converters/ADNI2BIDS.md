...
If the (Optional) parameter `--xml_path` is given, `adni-to-bids` will try to extract meta data information from the `xml_path` provided, and convert these fields into a json file `adni_xml_metadata.json`.
