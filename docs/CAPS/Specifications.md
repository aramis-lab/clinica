...
Given a file generated by a Clinica pipeline, the PyTorch tensor of the 2D slices where follows the same naming convention except that `_patchsize-<N>_stride-<M>_patch-<index>` is inserted between `keyN-<value>` and `_suffix` and  extension is replaced by `.pt`.
