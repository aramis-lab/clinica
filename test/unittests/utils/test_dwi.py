...
def test_rename_files(tmp_path):
    ...
    assert rename_files(
        "sub-01_ses-M000_dwi_space-b0_preproc.bval",
        {
            str(tmp_path / "foo/bar/baz.nii.gz"): "_space-b0_preproc.nii.gz",
            str(
                tmp_path / "foo/bar/sub-02_ses-M666_foo.nii.gz"
...
def test_extract_bids_identifier_from_filename(dwi_filename, expected):
    ...
    assert extract_bids_identifier_from_filename(dwi_filename) == expected
...
def test_extract_bids_identifier_from_filename_error():
    ...
    with pytest.raises(
        ...
        extract_bids_identifier_from_filename("foo/bar/sub-01_ses-M000_baz.foo.bar")
