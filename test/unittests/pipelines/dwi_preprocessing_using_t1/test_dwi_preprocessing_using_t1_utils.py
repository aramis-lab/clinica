...
def test_prepare_reference_b0(tmp_path, mocker):
    ...
    output_dir = tmp_path / "tmp" / hashlib.md5(str(dataset.dwi).encode()).hexdigest()
    assert reference_b0 == tmp_path / "reference_b0_volume.nii.gz"
    assert reference_dataset.dwi == output_dir / "sub-foo_ses-bar_dwi_merged.nii.gz"
    assert reference_dataset.b_values == output_dir / "sub-foo_ses-bar_dwi_merged.bval"
    assert reference_dataset.b_vectors == output_dir / "sub-foo_ses-bar_dwi_merged.bvec"
    assert sorted([p.name for p in tmp_path.iterdir()]) == [
    ...
    large_b_image = nib.load(output_dir / "sub-foo_ses-bar_dwi_large_b.nii.gz")
    assert_array_equal(large_b_image.get_fdata(), 4.0 * np.ones((5, 5, 5, 6)))
    large_b_values = np.loadtxt(output_dir / "sub-foo_ses-bar_dwi_large_b.bval")
    assert_array_equal(large_b_values, np.array([1000] * 6))
    large_b_vectors = np.loadtxt(output_dir / "sub-foo_ses-bar_dwi_large_b.bvec")
    assert large_b_vectors.shape == (3, 6)
    small_b_image = nib.load(output_dir / "sub-foo_ses-bar_dwi_small_b.nii.gz")
    assert_array_equal(small_b_image.get_fdata(), 4.0 * np.ones((5, 5, 5, 3)))
    merged_image = nib.load(output_dir / "sub-foo_ses-bar_dwi_merged.nii.gz")
    ...
    merged_b_values = np.loadtxt(output_dir / "sub-foo_ses-bar_dwi_merged.bval")
    assert_array_equal(merged_b_values, np.array([0] + [1000] * 6))
    merged_b_vectors = np.loadtxt(output_dir / "sub-foo_ses-bar_dwi_merged.bvec")
