# -*- coding: utf-8 -*-

#def connectome_construction_pipeline(
#        in_parcellation, configuration_file, lut_type, lut_path, in_tracks, connectome_metric,
#        working_directory, datasink_directory, in_scalar_image='', zero_diagonal=True):
#    """
#    Perform the construction of the subject connectome.
#
#    This function combine tracks obtained from tractography with a parcellated cortex in order to compute the
#    subject connectome as a connection matrix. This function first prepare a parcellated image for connectome
#    construction by modifying the image values; typically this involves making the parcellation intensities
#    increment from 1 to coincide with rows and columns of a matrix. The configuration file passed as the second
#    argument specifies the indices that should be assigned to different structures; examples of such configuration
#    files are provided in the MRtrix3 directory in src//dwi//tractography//connectomics//example_configs//. Then
#    a connectome matrix is generated from a streamlines file and the configurate node parcellation image.
#
#    Args:
#        in_parcellation (str): 3D image of the parcellated cortex
#        configuration_file (str): the MRtrix connectome configuration file specifying desired nodes & indices
#        lut_type (str): Type of lookup table should be 'freesurfer', 'aal', 'itksnap' or 'basic' otherwise
#        lut_path (str): Path of the lookup table. If type is 'FreeSurfer' then typically 'FreeSurferColorLUT.txt',
#            if it is 'aal' then typically 'ROI_MNI_V4.txt', if it is 'itksnap' the ntypically this include the ITK atlas
#            'LUT_GM.txt'. Otherwise, if it is 'basic' then this is a basic lookup table consisting of index / name pairs
#        in_tracks (str): the input track file
#        connectome_metric (str): specify the edge weight metric. Options are: count (default), meanlength, invlength,
#            invnodevolume, invlength_invnodevolume, mean_scalar. If mean_scalar is selected an associate scalar image
#            is espected in the in_scalar_image
#        in_scalar_image (str): provide the associated image for the mean_scalar metric
#        zero_diagonal (bool): If set True set all diagonal entries in the matrix to zero (default is True)
#
#    Returns:
#        out_connectome (str): The output .csv file containing edge weights store in a matrix.
#    """
#
#    import nipype.interfaces.io as nio
#    import nipype.interfaces.utility as niu
#    import nipype.pipeline.engine as pe
#    import os.path as op
#
#    # Inputs existence checking
#
#    inputs = [in_parcellation, configuration_file, lut_path, in_tracks, working_directory, datasink_directory]
#
#    for input_file in inputs:
#        if not op.exists(input_file):
#            raise IOError('file {} does not exist'.format(input_file))
#
#    if lut_type.lower() not in ['freesurfer', 'aal', 'itksnap', 'basic']:
#        raise IOError('lut_type should be in "freesurfer", "aal", "itksnap" or "basic"')
#
#    # if connectome_metric.lower() not in  ['count', 'meanlength', 'invlength', 'invnodevolume', 'invlength_invnodevolume', 'mean_scalar']:
#    #        raise IOError('connectome_metric should be in "count", "meanlength", "invlength", "invnodevolume", "invlength_invnodevolume", "mean_scalar"')
#    #    elif connectome_metric.lower()=='mean_scalar':
#    #        if not op.exists(in_scalar_image):
#    #            raise IOError('A "mean_scalar" connectome_metric value should be associated with a valid in_scalar_image input. file {} does not exist'.format(in_scalar_image))
#
#    if type(zero_diagonal) != bool:
#        raise IOError('zero_diagonal should be set to "True" or "False".')
#
#    # Nodes definition
#
#    datasource = pe.Node(interface=nio.DataGrabber(infields=[],
#                                                   outfields=['in_parcellation', 'configuration_file', 'lut_path',
#                                                              'in_tracks']), name='datasource')
#    datasource.inputs.template = '*'
#    datasource.inputs.field_template = dict(in_parcellation=in_parcellation,
#                                            configuration_file=configuration_file,
#                                            lut_path=lut_path,
#                                            in_tracks=in_tracks)
#    datasource.inputs.template_args = dict(in_parcellation=[[]],
#                                           configuration_file=[[]],
#                                           lut_path=[[]],
#                                           in_tracks=[[]])
#    datasource.inputs.sort_filelist = True
#
#    inputnode = pe.Node(niu.IdentityInterface(
#        fields=['in_parcellation', 'configuration_file', 'lut_path', 'in_tracks']),
#        name='inputnode')
#
#    label_config = pe.Node(interface=niu.Function(
#        input_names=['input_parcellation', 'config_in', 'lut_type', 'lut_path'], output_names=['out_image'],
#        function=labelconfig), name='label_config')
#    label_config.inputs.lut_type = lut_type
#
#    tck_to_connectome = pe.Node(interface=niu.Function(
#        input_names=['in_parcellation', 'in_tracks', 'scalar_image', 'zero_diagonal'], output_names=['out_connectome'],
#        function=tck_to_connectome), name='tck_to_connectome')
#    tck_to_connectome.inputs.zero_diagonal = zero_diagonal
#    tck_to_connectome.inputs.scalar_image = in_scalar_image
#    #    tck_to_connectome.inputs.metric = connectome_metric
#
#    outputnode = pe.Node(niu.IdentityInterface(
#        fields=['out_connectome']), name='outputnode')
#
#    datasink = pe.Node(nio.DataSink(), name='datasink')
#    datasink.inputs.base_directory = op.join(datasink_directory, 'connectome/')
#
#    wf = pe.Workflow(name='compute_connectome')
#    wf.base_dir = working_directory
#
#    wf.connect([
#        (datasource, inputnode, [('in_parcellation', 'in_parcellation'),
#                                 ('configuration_file', 'configuration_file'),
#                                 ('lut_path', 'lut_path'),
#                                 ('in_tracks', 'in_tracks')]),
#        (inputnode, label_config, [('in_parcellation', 'input_parcellation'),
#                                   ('configuration_file', 'config_in'),
#                                   ('lut_path', 'lut_path')]),
#        (inputnode, tck_to_connectome, [('in_tracks', 'in_tracks')]),
#        (label_config, tck_to_connectome, [('out_image', 'in_parcellation')]),
#        (tck_to_connectome, outputnode, [('out_connectome', 'out_connectome')]),
#        (tck_to_connectome, datasink, [('out_connectome', 'out_connectome')])
#    ])
#
#    return wf


def dwi_connectome_construction_pipeline(
        participant_id, session_id, caps_directory,
        working_directory=None,
        type_of_parcellation='FreeSurfer-Desikan', zero_diagonal=True,
        name="dwi_connectome_construction_pipeline"):
    """
    Perform the construction of the subject connectome.

    This function combine tracks obtained from tractography with a parcellated cortex in order to compute the
    subject connectome as a connection matrix. This function first prepare a parcellated image for connectome
    construction by modifying the image values; typically this involves making the parcellation intensities
    increment from 1 to coincide with rows and columns of a matrix. The configuration file passed as the second
    argument specifies the indices that should be assigned to different structures; examples of such configuration
    files are provided in the MRtrix3 directory in src//dwi//tractography//connectomics//example_configs//. Then
    a connectome matrix is generated from a streamlines file and the configurate node parcellation image.

    Args:
        in_parcellation (str): 3D image of the parcellated cortex
        configuration_file (str): the MRtrix connectome configuration file specifying desired nodes & indices
        type_of_parcellation (str): Path of the lookup table. If type is 'FreeSurfer' then typically 'FreeSurferColorLUT.txt',
            if it is 'aal' then typically 'ROI_MNI_V4.txt', if it is 'itksnap' the ntypically this include the ITK atlas
            'LUT_GM.txt'. Otherwise, if it is 'basic' then this is a basic lookup table consisting of index / name pairs
        in_tracks (str): the input track file
        in_scalar_image (str): provide the associated image for the mean_scalar metric
        zero_diagonal (bool): If set True set all diagonal entries in the matrix to zero (default is True).

    Inputnode:
        in_parcellation (str): Parcellation image.
        in_tracks (str): Whole-brain tractogram.

    Outputnode:
        out_parcellation (str): Parcellation with a remapping of the indices.
        out_connectome (str): The output .csv file containing edge weights store in a matrix.
    """

    import nipype.interfaces.io as nio
    import nipype.interfaces.utility as niu
    import nipype.pipeline.engine as pe
    import os.path as op
    import os

    inputnode = pe.Node(niu.IdentityInterface(
        fields=['in_parcellation', 'in_tracks']),
        name='inputnode')

    freesurfer_home = os.environ.get('FREESURFER_HOME', '')
    mrtrix_home = os.environ.get('MRTRIX_HOME', '')

    if type_of_parcellation.lower() == 'freesurfer-desikan':
        in_lut_path = op.join(freesurfer_home, 'FreeSurferColorLUT.txt')
        target_lut_path = op.join(mrtrix_home, 'src', 'connectome', 'tables', 'fs_default.txt')
        name_parcellation_file = participant_id + '_' + session_id + '_parcellation-desikan_connectome.nii.gz'
        name_connectome_file = participant_id + '_' + session_id + '_parcellation-desikan_connectome.csv'
    elif type_of_parcellation.lower() == 'freesurfer-destrieux':
        in_lut_path = op.join(freesurfer_home, 'FreeSurferColorLUT.txt')
        target_lut_path = op.join(mrtrix_home, 'src', 'connectome', 'tables', 'fs_a2009s.txt')
        name_parcellation_file = participant_id + '_' + session_id + '_parcellation-destrieux_connectome.nii.gz'
        name_connectome_file = participant_id + '_' + session_id + '_parcellation-destrieux_connectome.csv'
    else:
        raise IOError('type_of_parcellation should be FreeSurfer-Desikan or FreeSurfer-Destrieux (it is ' + type_of_parcellation.lower()  + ')')

    if type(zero_diagonal) != bool:
        raise IOError('zero_diagonal should be set to "True" or "False".')

    inputnode = pe.Node(niu.IdentityInterface(
        fields=['in_parcellation', 'in_tracks']),
        name='inputnode')

    convert_label = pe.Node(interface=niu.Function(
        input_names=['in_connectome', 'in_lut_path', 'target_lut_path', 'name_output_file'], output_names=['out_connectome'],
        function=labelconvert), name='convert_label')
    convert_label.inputs.in_lut_path = in_lut_path
    convert_label.inputs.target_lut_path = target_lut_path
    convert_label.inputs.name_output_file = name_parcellation_file

    create_connectome = pe.Node(interface=niu.Function(
        input_names=['in_parcellation', 'in_tracks', 'zero_diagonal', 'name_output_file'],
        output_names=['out_connectome'],
        function=tck_to_connectome), name='create_connectome')
    create_connectome.inputs.zero_diagonal = zero_diagonal
    create_connectome.inputs.name_output_file = name_connectome_file

    outputnode = pe.Node(niu.IdentityInterface(
        fields=['out_parcellation', 'out_connectome']), name='outputnode')

    datasink = pe.Node(nio.DataSink(), name='datasink')
    datasink.inputs.base_directory = op.join( caps_directory, 'subjects', participant_id, session_id, 'dwi')

    wf = pe.Workflow(name='compute_connectome', base_dir=working_directory)
    wf.connect([
        # Convert connectome:
        (inputnode, convert_label, [('in_parcellation', 'input_parcellation')]),
        # Create connectome:
        (inputnode,     create_connectome, [('in_tracks', 'in_tracks')]),
        (convert_label, create_connectome, [('out_connectome', 'in_parcellation')]),
        # Output node:
        (create_connectome, outputnode, [('out_connectome', 'out_connectome')]),
        # Datasink:
        (create_connectome, datasink, [('out_connectome', 'out_connectome')])
    ])

    return wf


#def labelconfig(input_parcellation, config_in, lut_path):
#    """
#    TODO
#    """
#    import os.path as op
#    import os
#
#    out_image = op.abspath('parcellation_config.nii')
#
#    cmd = 'labelconfig -lut_freesurfer ' + lut_path + ' ' + input_parcellation + ' ' + config_in + ' ' + out_image
#    os.system(cmd)
#
#    return out_image

#def tck2connectome(in_parcellation, in_tracks, scalar_image='', zero_diagonal=True, name_output_file=None):
#    """
#    TODO
#    """
#    import subprocess
#    import os.path as op
#    out_connectome = op.abspath('connectome.csv')
#    if zero_diagonal:
#        diag = '-zero_diagonal '
#    else:
#        diag = ''
##    if metric == 'mean_scalar':
##        if scalar_image:
##            image = '-image ' + scalar_image + ' '
##        else:
##            raise IOError('A metric "mean_scalar" should be associated with a valid scalar_image input.')
##    else:
##        image = ''
#    image = ''
#    cmd = ' tck2connectome ' + diag + ' ' + image + in_tracks + ' ' + in_parcellation + ' ' + out_connectome
#    subprocess.call([cmd], shell=True)
#    if not op.exists(out_connectome):
#        raise IOError('file {} does not exist, cmd is : {}'.format(out_connectome, cmd))
#    return out_connectome
#


def tck_to_connectome(in_parcellation, in_tracks, zero_diagonal=True, name_output_file=None):
    """
    TODO
    """
    import os.path as op
    import os

    if name_output_file is None:
        out_connectome = op.abspath('connectome.csv')
    else:
        outfile = op.abspath(name_output_file)
    if zero_diagonal:
        diag = '-zero_diagonal '
    else:
        diag = ''

    cmd = 'tck2connectome ' + diag + ' ' + in_tracks + ' ' + in_parcellation + ' ' + out_connectome
    os.system(cmd)

    return out_connectome



def labelconvert(in_connectome, in_lut_path, target_lut_path, name_output_file=None):
    """
    Convert a connectome to a usable one for MRtrix.

    Thus functions converts a connectome node image from one lookup table to another. Typical usage is to convert
    a parcellation image provided by some other software, based on the lookup table provided by that software,
    to conform to a new lookup table, particularly one where the node indices increment from 1, in preparation
    for connectome construction; examples of such target lookup table files are provided in src/connectome/tables/.

    Args:
        in_connectome (str): The input image.
        in_lut_path (str): Path to the connectome lookup table for the input image.
        target_lut_path (str): The target connectome lookup table for the output image.
        name_output_file (Optional[str]): Name of the output connectome file (default=connectome.nii.gz).

    Returns:
        out_connectome (str): The output connectome for the tck2connectome command.
    """
    import os.path as op
    import os


    if name_output_file is None:
        outfile = op.abspath('connectome.nii.gz')
    else:
        outfile = op.abspath(name_output_file)

    assert(op.isfile(in_connectome))

    out_connectome = op.abspath(name_output_file)

    cmd = 'labelconvert ' + in_connectome + ' ' + in_lut_path + ' ' + target_lut_path + ' ' + out_connectome
    os.system(cmd)

    return out_connectome
