...
def dti_based_analysis_pipeline(participant_id, session_id, caps_directory,
                                working_directory=None, atlas_name=None, name="dti_based_analysis_pipeline"):
    """
    Perform tracts analysis according to a white matter atlas using a tensor-derived scalar image.
    ...
    antsRegistrationSyNQuick. Then, the estimated deformation is applied to MD-map, AD-map and RD-map. Finally,
    ...
    Outputnode:
        ...
        out_b_spline_transform (str): BSpline transformation obtained by antsRegistrationSyNQuick
            after registration towards <atlas_name>.
        out_registered_fa (str):
        out_registered_md (str):
        out_registered_ad (str):
        out_registered_rd (str):
        out_fa_statistics (str):
        out_md_statistics (str):
        out_ad_statistics (str):
        out_rd_statistics (str):
    """
    import tempfile
    import os
    import nipype.interfaces.io as nio
...
#        atlas_labels=os.path.join(fsl_dir, 'data', 'atlases', 'JHU', atlas_name + '-1mm.nii.gz')
    ...
    wf.connect([
        ...
        (apply_ants_registration_for_rd, scalar_analysis_rd, [('out_deformed_image', 'input_image')]),
        # Outputnode:
        (ants_registration,              outputnode, [('image_warped', 'out_deformed_ad'),
                                                      ('affine_matrix', 'out_affine_transform'),
