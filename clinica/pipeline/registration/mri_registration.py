...
def t1_b0_registration_pipeline(
                 datasink_directory, working_directory=None,
    ...
    Inputnode:
        ...
        in_preprocessed_dwi (str): File containing the preprocessed DWI dataset.
            It assumes that the reference b0 is the first volume in the
            dataset (which is the case if you are using Clinica).
    ...
    Example:
        ...
        >>> t1_b0_registration = t1_b0_registration_pipeline(datasink_directory='/path/to/output/results')
    ...
    destrieux_in_diffusion_space = pe.Node(interface=niu.Function(
        input_names=['in_image', 'in_mrtrix_matrix', 'name_output_image'],
        output_names=['out_deformed_image'], function=apply_mrtrix_transform_without_resampling), name='destrieux_in_native_space')
    destrieux_in_diffusion_space.inputs.name_output_image = 'destrieux_parcellation_in_diffusion_space.nii.gz'

    outputnode = pe.Node(niu.IdentityInterface(
        fields=['out_registered_t1', 'out_wm_mask_in_diffusion_space', 'out_mrtrix_matrix', 'out_desikan_in_diffusion_space', 'out_destrieux_in_diffusion_space']),
        name='outputnode')

    datasink = pe.Node(nio.DataSink(), name='datasink')
    datasink.inputs.base_directory = join(datasink_directory, 'registration_t1_to_b0/')
    ...
    wf.connect([
        ...
        (inputnode, upsample_b0_mask, [('roi_file', 'in_file')]),
