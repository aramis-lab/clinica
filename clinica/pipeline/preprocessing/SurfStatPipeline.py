...
def SurfStatGruopAnalysis(ContrastLinearModel,Format, CSVFilename, PATH_TO_RECON_ALL_OUTPUTS, a_required_path):
    """
    ...
    def runmatlab(ContrastLinearModel, Format, CSVFilename, PATH_TO_RECON_ALL_OUTPUTS, a_required_path):
        ...
        NipypeSurfStat('%s', '%s', '%s', '%s', '%s')"""%(ContrastLinearModel, Format, 
                                     CSVFilename, PATH_TO_RECON_ALL_OUTPUTS, a_required_path )  # here, we should define the inputs for the matlab function that you want to use
