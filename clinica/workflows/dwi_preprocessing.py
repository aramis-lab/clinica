...
def prepare_b0(num_b0s, low_bval=5, name="prepare_b0"):
    ...
    inputnode = pe.Node(
        interface=niu.IdentityInterface(fields=["in_dwi", "in_bvec", "in_bval"]),
        name="inputnode",
    )

    b0_dwi_split = pe.Node(
        niu.Function(
    ...
    b0_avg = pe.Node(
        niu.Function(
            input_names=["in_file"], output_names=["out_file"], function=b0_average
        ),
        name="b0_average",
    )
...
def dwi_flirt(name="DWICoregistration", excl_nodiff=False, flirt_param={}):
    ...
    wf.connect(
        [
            ...
            (inputnode, n4, [("reference", "input_image"), ("ref_mask", "mask_image")]),
...
def hmc_pipeline(name="motion_correct"):
    ...
    insmat = pe.Node(
        niu.Function(
            input_names=["inlist", "volid"], output_names=["out"], function=insert_mat
        ),
    ...
    outputnode = pe.Node(
        niu.IdentityInterface(fields=["out_file", "out_bvec", "out_xfms", "mask_B0"]),
        name="outputnode",
    )
...
head-motion correction)
    ...
    inputnode = pe.Node(
        niu.IdentityInterface(fields=["in_file", "in_bval", "in_mask", "in_xfms"]),
        name="inputnode",
    )
    ...
    wf.connect(
        [
            ...
            (flirt, mult, [(("outputnode.out_xfms", _xfm_jacobian), "operand_value")]),
...
def eddy_fsl_pipeline(epi_param, name="eddy_fsl"):
    ...
    list_b0 = pe.Node(
        niu.Function(
            input_names=["in_bval"], output_names=["out_idx"], function=b0_indices
        ),
