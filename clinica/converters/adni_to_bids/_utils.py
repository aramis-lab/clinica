...
def _filter_subj_bids(
    ...
    if location == "ADNIMERGE.csv":
        df_files["RID"] = df_files["PTID"].apply(
            lambda x: _remove_space_and_symbols(x)[4:]
        )
    else:
...
def _remove_space_and_symbols(data: Union[str, list[str]]) -> Union[str, list[str]]:
    ...
    if isinstance(data, list):
        return [_remove_space_and_symbols(d) for d in data]
...
def _create_file(
    ...
    if image.Is_Dicom:
        ...
        if not nifti_exists or not dwi_bvec_and_bval_exist:
            log_and_warn(
                f"{logging_header} Conversion with dcm2niix failed for subject {subject} and session {session}",
                UserWarning,
            )
            return np.nan
