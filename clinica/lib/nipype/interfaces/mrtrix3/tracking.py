...
class TckSiftInputSpec(MRTrix3BaseInputSpec):
    ...
    in_fod = File(
        ...
        mandatory=True,
        position=1,
        desc=('input image containing the spherical harmonics of '
              'the fibre orientation distributions'))
    out_tracks = File(
        'out_tracks.tck',
        argstr='%s',
        mandatory=True,
...
class TckSift(MRTrix3Base):
    ...
    >>> import nipype.interfaces.mrtrix3 as mrt
    >>> tr_sift = mrt.TckSift()
    >>> tr_sift.inputs.in_tracks = 'in_tracks.tck'
    >>> tr_sift.inputs.in_fod = 'in_fod.mif'
    >>> tr_sift.cmdline                               # doctest: +ELLIPSIS
    'tcksift in_tracks.tck in_fod.mif out_tracks.tck'
    >>> tr_sift.run()                                 # doctest: +SKIP
    """
