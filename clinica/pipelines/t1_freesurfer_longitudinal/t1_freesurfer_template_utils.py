...
def save_to_caps(source_dir, image_id, list_session_ids, caps_dir, overwrite_caps):
    ...
    if os.path.isfile(representative_source_file):
        ...
        try:
            os.unlink(os.path.join(os.path.expanduser(source_dir), 'fsaverage'))
            os.unlink(os.path.join(os.path.expanduser(source_dir), 'lh.EC_average'))
            os.unlink(os.path.join(os.path.expanduser(source_dir), 'rh.EC_average'))
            for session_id in list_session_ids:
                os.unlink(os.path.join(os.path.expanduser(source_dir), participant_id + '_' + session_id))
