...
class T1FreeSurferVisualizer(ce.CmdParser):
    ...
    def run_command(self, args):
        ...
        subject_id = f"{participant_id}_{session_id}"

        cprint(
            f"Visualizing outputs of t1-freesurfer pipeline for {participant_id} at session {session_id}."
        )
        caps_directory = Path(args.caps_directory)
        caps_participant = (
            caps_directory
            / "subjects"
            / participant_id
        ...
        if not caps_participant.exists():
            if (caps_directory / "subjects" / participant_id / session_id).exists():
                error_msg = (
                ...
                error_msg = (
                    ...
                    f"present in CAPS {caps_directory}"
                )
            raise RuntimeError(error_msg)

        # Check that freesurfer has run properly
        log_file = caps_participant / "scripts" / "recon-all-status.log"
        if log_file.is_file():
        ...
        files = {
            "nu": caps_participant / "mri" / "nu.mgz",
            "aseg": caps_participant / "mri" / "aseg.mgz",
            "lh_white": caps_participant / "surf" / "lh.white",
            "lh_pial": caps_participant / "surf" / "lh.pial",
            "rh_white": caps_participant / "surf" / "rh.white",
            "rh_pial": caps_participant / "surf" / "rh.pial",
        }
        if (
            len((files_not_found := [f for _, f in files.items() if not f.exists()]))
            > 0
        ):
            error_string = (
                "The following file(s) has(ve) not been found :\n"
