...
class SuvrReferenceRegion(str, Enum):
    """Possible values for SUVR reference regions."""
...
def get_suvr_mask(region: Union[str, SuvrReferenceRegion]) -> Path:
    ...
    """
    masks_dir = Path(__file__).resolve().parent.parent / "resources" / "masks"

    return masks_dir / _get_label_filename_from_suvr_region(SuvrReferenceRegion(region))


def _get_label_filename_from_suvr_region(region: SuvrReferenceRegion) -> str:
    if region == SuvrReferenceRegion.PONS:
        return "region-pons_eroded-6mm_mask.nii.gz"
    if region == SuvrReferenceRegion.CEREBELLUMPONS:
...
def _read_psf_dataframe(filename: PathLike) -> pd.DataFrame:
    valid_columns = {
        "participant_id",
        "session_id",
        "acq_label",
