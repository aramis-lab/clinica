...
class DwiPreprocessingUsingT1(cpe.Pipeline):
    ...
    def check_pipeline_parameters(self):
        ...
        if self.parameters["low_bval"] < 0:
            raise ValueError(
                f"{Fore.RED}The low_bval is equals to {self.parameters['low_bval']}: "
                f"it should be zero or close to zero.{Fore.RESET}"
            )

        if self.parameters["low_bval"] > 100:
            cprint(
                f"{Fore.YELLOW}Warning: The low_bval parameter is {self.parameters['low_bval']}: "
                f"it should be close to zero.{Fore.RESET}"
            )
        ...
        if self.parameters["use_cuda_8_0"] and self.parameters["use_cuda_9_1"]:
            raise ClinicaException(
                f"\n{Fore.RED}[Error] You must choose between CUDA 8.0 or CUDA 9.1, not both.{Fore.RESET}"
            )
        ...
        if self.parameters["use_cuda_8_0"]:
            if not is_binary_present("eddy_cuda8.0"):
                raise ClinicaMissingDependencyError(
                    f"{Fore.RED}\n[Error] FSL eddy with CUDA 8.0 was set but Clinica "
                    f"could not find eddy_cuda8.0 in your PATH environment.{Fore.RESET}"
                )

        if self.parameters["use_cuda_9_1"]:
            if not is_binary_present("eddy_cuda9.1"):
                raise ClinicaMissingDependencyError(
                    f"{Fore.RED}\n[Error] FSL eddy with CUDA 9.1 was set but Clinica "
                    f"could not find eddy_cuda9.1 in your PATH environment.{Fore.RESET}"
                )
