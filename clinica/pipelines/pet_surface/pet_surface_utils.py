...
def remove_nan(volname):
    ...
    data = np.nan_to_num(nifti_in.get_fdata())
...
def suvr_normalization(pet_path, mask):
    ...
    eroded_mask = eroded_mask_nifti.get_fdata()
...
def weighted_mean(in_surfaces):
    ...
    for i in range(len(in_surfaces)):
        ...
        data_normalized += current_surf.get_fdata() * coefficient[i]
...
def produce_tsv(pet, atlas_files):
    ...
    lh_pet_mgh = np.squeeze(nib.load(pet[0]).get_fdata())
    rh_pet_mgh = np.squeeze(nib.load(pet[1]).get_fdata())
