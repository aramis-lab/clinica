...
def make_label_conversion(gtmsegfile, csv):
    ...
    if list(convert_lut.columns.values) != ['REGION', 'SOURCE', 'DST']:
        raise Exception('CSV file ' + csv + ' is not in the correct format. Columns should be : REGION, SOURCE, DST')
    ...
    for i in range(old_labels.size):
        ...
        if index.size == 0:
            raise Exception('Could not find label '
                            + str(old_labels[i])
                            + ' on conversion table. Add it manually in csv file to correct error')
        else:
    ...
    if not isclose(1.0, sum_voxel_mean):
        raise Exception('Problem during parcellation : the mean sum of a voxel across 4th dimension is '
                        + str(sum_voxel_mean)
                        + ' instead of 1.0')
...
def runApplyInverseDeformationField(target,
    ...
    """ % (target, deformation_field, img, prefix)
    ...
    if not os.path.exists(output_file):
        raise IOError('Something went wrong, please check ' + os.path.abspath(matlab.inputs.logfile) + ' for more information')
    return output_file
...
def suvr_normalization(pet_path, mask):
    ...
    if mask_size == 0:
        raise Exception('Number of non-zero value of mask is 0. A problem occured when moving the eroded mask from MNI to gtmsegspace')
...
def suvr_normalization2(pet_path, gtmseg_path, pet_type):
    ...
    suvr_normalization is a way of getting suvr from your pet image, based on the segmentation performed by
    gtmsegmentation. The Standard Uptake Value ratio is computed by dividing the whole PET volume by the mean value
    observed in the pons. The mask of the pons is eroded 3 times by a kernel of 3x3x3, in order to have a more
    homogeneous area. In the case of AV45 PET, normalization is realized on pons and cerebellum.

    Args:
        (string) pet_path     : path to the Nifti volume containing PET scan, realigned on upsampled T1 whose
    ...
    el.astype(numpy.int)
    N_iter = 5
    eroded_normalization_mask = ndimage.binary_erosion(normalization_mask, el, N_iter)

    # check that eroded mask is not null
    mask_size = sum(sum(sum(eroded_normalization_mask)))
    while mask_size < 1:
        N_iter -= 1
        eroded_normalization_mask = ndimage.binary_erosion(normalization_mask, el, N_iter)
        mask_size = sum(sum(sum(eroded_normalization_mask)))
        if N_iter == 0 and mask_size == 0:
            raise Exception('Could not find reference structure in ' + gtmseg_path)

    # Mask non-Pons values to determine mean uptake value
    pons_pet_activity = eroded_normalization_mask * pet_data
    mean_pons_pet_activity = sum(sum(sum(pons_pet_activity))) / sum(sum(sum(eroded_normalization_mask)))
...
def vol2surf(volume, surface, subject_id, session_id, caps_dir, gtmsegfile):
    ...
    # execute vol2surf
    output = os.path.abspath('./' + hemi + '.projection_' + os.path.basename(surface) + '.mgh')
    cmd = 'mri_vol2surf'
    cmd += ' --mov ' + volume
    cmd += ' --o ' + output
    cmd += ' --surf ' + surfname
...
def weighted_mean(in_surfaces):
    ...
    if len(in_surfaces) != 7:
        raise Exception('There should be 7 surfaces at this point of the pipeline, but found '
                        + str(len(in_surfaces))
    ...
    hemi_projection = nib.MGHImage(data_normalized, affine=sample.affine, header=sample.header)
...
def fsaverage_projection(projection, subject_id, caps_dir, session_id, fwhm):
    ...
    if not os.path.exists(os.path.join(os.path.expandvars('$SUBJECTS_DIR'), 'fsaverage')):
        shutil.copytree(os.path.join(subjects_dir_backup, 'fsaverage'),
    ...
    out_fsaverage = os.path.abspath('./fsaverage_fwhm-' + str(fwhm) + '_' + os.path.basename(projection))
...
def produce_tsv(pet, atlas_files):
    ...
    for atlas in atlas_files:
        ...
        annot_atlas_left = nib.freesurfer.io.read_annot(atlas_files[atlas]['lh'], orig_ids=False)
        annot_atlas_left[0][annot_atlas_left[0] == -1] = 0
        annot_atlas_right = nib.freesurfer.io.read_annot(atlas_files[atlas]['rh'], orig_ids=False)
        ...
        final_tsv.to_csv(filename_atlas_tsv, sep='\t', index=False, columns=['index',
                                                                             'label_name',
                                                                             'mean_scalar'])
