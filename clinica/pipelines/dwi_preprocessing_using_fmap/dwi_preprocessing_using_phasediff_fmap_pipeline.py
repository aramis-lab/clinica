...
class DwiPreprocessingUsingPhaseDiffFMap(cpe.Pipeline):
    ...
    def check_pipeline_parameters(self):
        ...
        if self.parameters["low_bval"] < 0:
            raise ValueError(
                f"{Fore.RED}The low_bval is equals to {self.parameters['low_bval']}: it should be zero or close to zero.{Fore.RESET}"
            )

        if self.parameters["low_bval"] > 100:
            cprint(
                f"{Fore.YELLOW}Warning: The low_bval parameter is {self.parameters['low_bval']}: it should be close to zero.{Fore.RESET}"
            )
        ...
        if self.parameters["use_cuda_8_0"]:
            if not is_binary_present("eddy_cuda8.0"):
                raise ClinicaMissingDependencyError(
                    ...
                    "eddy_cuda8.0 in your PATH environment.{Fore.RESET}"
                )
        ...
        if self.parameters["use_cuda_9_1"]:
            if not is_binary_present("eddy_cuda9.1"):
                raise ClinicaMissingDependencyError(
                    ...
                    "eddy_cuda9.1 in your PATH environment.{Fore.RESET}"
                )
    ...
    def get_input_fields(self):
        ...
        Returns:
            List[str]: The list of inputs for the DwiPreprocessingUsingT1 pipeline namely:
                ...
                * fmap_magnitude: Path of the magnitude (1st) image in BIDS format
