...
def init_input_node(
    ...
    try:
        ...
        error_msg = (
            f"{Fore.RED}[{now}] Error: Number of DWIs, b-vals and b-vecs mismatch "
    ...
    if img_phasediff.shape != img_magnitude.shape:
        now = datetime.datetime.now().strftime("%H:%M:%S")
        error_msg = (
            f"{Fore.RED}[{now}] Error: Headers of PhaseDiff and Magnitude1 are not the same "
            f"for {image_id.replace('_', ' | ')} ({img_phasediff.shape} vs {img_magnitude.shape}){Fore.RESET}"
        )
        cprint(error_msg)
