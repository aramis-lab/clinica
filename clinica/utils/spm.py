...
def _get_platform_dependant_matlab_command_for_spm_standalone(
    ...
    # todo : not always spm12 -> can be spm25... do regex ? can be a solution to check path by the way
    user_system = platform.system().lower()
    if user_system.startswith("darwin"):
        return f"cd {spm_standalone_home} && ./run_spm25.sh {mcr_home} script"
    if user_system.startswith("linux"):
        return f"{spm_standalone_home / 'run_spm25.sh'} {mcr_home} script"
    raise SystemError(
...
def configure_nipype_interface_to_work_with_spm_standalone() -> None:
    ...
    # todo : when using the function alone we get to this we get the error (on MACOS): *** OSError: This docstring was not generated by Nipype!
    # See this issue on nipype : https://neurostars.org/t/oserror-this-docstring-was-not-generated-by-nipype/29837/3
