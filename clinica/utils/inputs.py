...
def clinica_list_of_files_reader(participant_ids,
                                 session_ids,
    ...
    Args:
        ...
        raise_exception (boolean): Raise Exception or not

    Returns:
        List of list of found files following order of `list_information`
    """
    ...
    for info_file in list_information:
        try:
            list_found_files.append(
                clinica_file_reader(participant_ids, session_ids, bids_or_caps_directory, info_file, True)
            )
        except ClinicaException as e:
            list_found_files.append([])
            all_errors.append(e)
