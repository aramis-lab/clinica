...
def get_img_metadata(img):
    ...
    assert all(p.tag == 'protocol' and 'term' in p.attrib.keys() 
               for p in proto)
    
    proto_d = {
        'MRI_' + "_".join(p.attrib['term'].split()).upper(): p.text
        for p in proto
    }
    
    # replace confusing '|' (reserved for separation btw elements) with space (only 1 manufacturer with this apparently but...)
    proto_d = {k: v.replace('|', ' ') if isinstance(v, str) else v # there are some None (not str)
               for k, v in proto_d.items()}
    
    return {
        
        'IMAGE_ORIG_ID': xml_check_and_get_text(img[0], 'imageUID', cast=int),
        'IMAGE_ORIG_SEQ': xml_check_and_get_text(img[1], 'description'), #.replace('|', ' '),
...
def parse_xml_file(xml_path):
    ...
    assert xml_check_and_get_text(proj[0], 'projectIdentifier') == 'ADNI', 'Not ADNI cohort'
    ...
    if len(study) == 7:
        ...
        series_meta = xml_check(series[-1], 'seriesLevelMeta', {3,4}) # we can have 1 or 2 related imgs...
        ...
        if len(series_meta) == 4:
            ...
            orig_img_bis = xml_check(rel_img_bis[0], 'originalRelatedImage', len(orig_img))
            orig_img = [orig_img, orig_img_bis]
        
        # specific
        pipe_name = xml_check_and_get_text(xml_check(series_meta[0], 'annotation', 1)[0], 'text')
        ...
        derived_d = {
            ...
            'IMAGE_PROC_DESC': xml_check_and_get_text(deriv[1], 'processedDataLabel').strip(),
        }
        
        assert xml_check_and_get_text(deriv[2], 'imageType') == 'image volume', 'imageType'
        assert xml_check_and_get_text(deriv[3], 'tissue') == 'All', 'tissue'
        assert xml_check_and_get_text(deriv[4], 'hemisphere') == 'Both', 'hemis'
        assert xml_check_and_get_text(deriv[5], 'anatomicStructure') == 'Brain', 'anat struct'
        assert xml_check_and_get_text(deriv[6], 'registration') == 'native', 'registration not native'
        ...
        if deriv[-1].tag == 'creationDate':
            assert xml_check_and_get_text(deriv[-1], 'creationDate') == '0000-00-00', 'creaDate'
        else:
            assert xml_check_and_get_text(deriv[-2], 'creationDate') == '0000-00-00', 'creaDate'
    ...
    if proc_img_rating is not None:
        assert proc_img_rating == orig_img_d.get('IMAGE_ORIG_RATING', None), 'Image rating not consistent'
    ...
    row_d = {
        ...
        'VISIT_AGE': xml_check_and_get_text(study[1], 'subjectAge', cast=float), # to join visit ?
        # ageQualifier ??
...
def create_mri_meta_df(mri_meta, imgs):
    import numpy as np
    import pandas as pd
    import json
    ...
    mri_meta['T1w_scan_id'] = mri_meta['IMAGE_PROC_ID'].combine_first(mri_meta['IMAGE_ORIG_ID'])

    # Index
    mri_meta.set_index(['T1w_scan_id', 'ID','GENDER','VISIT_DESC','VISIT_AGE','SCAN_DATE',
                        'STUDY_ID','SERIES_ID'], inplace=True)
...
def create_json_metadata(mri_meta):
    ...
    mri_meta.reset_index(drop=True).iloc[0:2].to_json("output_file.json", orient="records", lines=False, indent=4)
