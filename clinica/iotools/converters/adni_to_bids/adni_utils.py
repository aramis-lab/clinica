...
def _check_existing_images(
    ...
    existing_images = list(filter(rgx.search, [f.name for f in folder.iterdir()]))
    return [folder / image for image in existing_images]
...
def _update_existing_images(
    ...
    if not existing_paths:
        ...
        cprint(f"Removing old images : {existing_paths}", lvl="info")
        for path_to_unlink in existing_paths:
            (path_to_unlink).unlink()
        return True
    cprint(
        f"There already exist images : {existing_paths}. "
...
def _remove_files_with_unsupported_suffixes(
    ...
    rgx = re.compile(r"|".join(suffixes))
    invalid_suffix_images = list(filter(rgx.search, [f.name for f in folder.iterdir()]))
...
def create_file(
    ...
    to_update = _update_existing_images(
        _check_existing_images(
            output_path,
