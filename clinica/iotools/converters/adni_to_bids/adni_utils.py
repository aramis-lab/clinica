...
def create_file(
    ...
    rgx_existing = re.compile(
        f"{modality_specific[modality]['output_filename']}" + r"|magnitude|phase"
    )
    existing_images = list(filter(rgx_existing.search, os.listdir(output_path)))

    for image_to_remove in existing_images:
        if not mod_to_update:
            cprint(
                f"There exists already images for modality {modality} : {existing_images}. "
    ...
    if image.Is_Dicom:
        ...
        invalid_rgx = re.compile(r"ADC|real|imaginary")
        invalid_suffix_images = list(
            filter(invalid_rgx.search, os.listdir(output_path))
        )

        for image_to_remove in invalid_suffix_images:
            cprint(
                f"Image with bad suffix 'ADC', 'real' or 'imaginary' was generated by dcm2niix "
                f"for subject {subject} and session {session}. This image will NOT be converted as the"
                f"suffix is not supported by Clinica.",
                lvl="warning",
            )
            cprint(f"Removing image {output_path / image_to_remove}", lvl="info")
            (output_path / image_to_remove).unlink()
