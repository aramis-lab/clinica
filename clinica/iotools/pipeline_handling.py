...
def _get_label_list(
    ...
    if pipeline == PipelineNameForMetricExtraction.T1_FREESURFER:
        return [
            f"t1-freesurfer_{atlas_name}_ROI-{replace_special_characters_with_symbol(roi_name, symbol='_')}_{'volume' if metric == 'segmentationVolumes' else metric}"
    ...
    if pipeline in (
        ...
        return [
            f"{pipeline.value}_{group}_{atlas_name}{additional_desc}_ROI-{replace_special_characters_with_symbol(roi_name, symbol='_')}_intensity"
            for roi_name in atlas_df.label_name.values
        ]
    if pipeline == PipelineNameForMetricExtraction.DWI_DTI:
        prefix = "dwi-dti_"
